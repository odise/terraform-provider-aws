...
func TestAccAWSCloudFormationStackSet_dataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:  func() { testAccPreCheck(t) },
		Providers: testAccProviders,
		Steps: []resource.TestStep{
			resource.TestStep{
				Config: testAccCheckAwsCloudFormationStackSetDataSourceConfig_basic,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "outputs.%", "1"),
					resource.TestMatchResourceAttr("data.aws_cloudformation_stack_set.network", "outputs.VPCId",
						regexp.MustCompile("^vpc-[a-z0-9]{8}$")),
					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "capabilities.#", "0"),
					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "disable_rollback", "false"),

					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "parameters.%", "1"),
					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "parameters.CIDR", "10.10.10.0/24"),

					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "tags.%", "2"),
					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "tags.Name", "Form the Cloud"),
					resource.TestCheckResourceAttr("data.aws_cloudformation_stack_set.network", "tags.Second", "meh"),
				),
			},
		},
	})
...
const testAccCheckAwsCloudFormationStackSetDataSourceConfig_basic = `
resource "aws_cloudformation_stack_set" "cfs" {
...
data "aws_cloudformation_stack_set" "network" {
  name = "${aws_cloudformation_stack_set.cfs.name}"
}
`
