...
func TestAccAWSCloudFormationStackSet_importBasic(t *testing.T) {
	stackName := fmt.Sprintf("tf-acc-test-basic-%s", acctest.RandString(10))

	resourceName := "aws_cloudformation_stack_set.network"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSCloudFormationStackSetConfig(stackName),
			},
			{
...
func TestAccAWSCloudFormationStackSet_basic(t *testing.T) {
	var stack cloudformation.StackSet
	stackName := fmt.Sprintf("tf-acc-test-basic-%s", acctest.RandString(10))
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudFormationStackSetExists("aws_cloudformation_stack_set.network", &stack),
				),
			},
		},
	})
...
func TestAccAWSCloudFormationStackSet_yaml(t *testing.T) {
	var stack cloudformation.StackSet
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAWSCloudFormationConfigStackSet_yaml(stackName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCloudFormationStackSetExists("aws_cloudformation_stack_set.yaml", &stack),
				),
			},
		},
	})
...
func testAccCheckCloudFormationStackSetExists(n string, stack *cloudformation.StackSet) resource.TestCheckFunc {
	...
	return func(s *terraform.State) error {
		...
		resp, err := conn.DescribeStackSet(params)
		if err != nil {
			fmt.Errorf("CloudFormation stack set resp: %q", resp)
			return err
		}
		/*
			if len(resp.Stacks) == 0 {
				return fmt.Errorf("CloudFormation stack set not found")
			}
		*/
		return nil
	}
...
func testAccCheckAWSCloudFormationStackSetDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		if err != nil {
			fmt.Errorf("CloudFormation stack set resp: %q", resp)
			fmt.Errorf("CloudFormation stack set error %q", err.Error())
			return err
		}
		/*
...
func testAccAWSCloudFormationStackSetConfig(stackName string) string {
	return fmt.Sprintf(`
resource "aws_cloudformation_stack_set" "network" {
  name = "%s"
  template_body = <<STACK
{
  "Resources" : {
...
func testAccAWSCloudFormationConfigStackSet_yaml(stackName string) string {
	return fmt.Sprintf(`
resource "aws_cloudformation_stack_set" "yaml" {
  name = "%s"
  template_body = <<STACK
Resources:
  MyVPC:
...
func testAccAWSCloudFormationStackSetConfig_defaultParams(stackName string) string {
	return fmt.Sprintf(`
resource "aws_cloudformation_stack_set" "asg-demo" {
  name = "%s"
  template_body = <<BODY
{
    "Parameters": {
...
var testAccAWSCloudFormationStackSetConfig_allAttributesWithBodies_tpl = `
resource "aws_cloudformation_stack_set" "full" {
  name = "%s"
  template_body = <<STACK
{
  "Parameters" : {
...
`
func testAccAWSCloudFormationStackSetConfig_withParams(stackName string) string {
	return fmt.Sprintf(
...
func testAccAWSCloudFormationStackSetConfig_withParams_modified(stackName string) string {
	return fmt.Sprintf(
		tpl_testAccAWSCloudFormationConfig_withParams,
		stackName,
		"12.0.0.0/16")
...
func testAccAWSCloudFormationStackSetConfig_templateUrl_withParams(rName, bucketKey, vpcCidr string) string {
	return fmt.Sprintf(`
